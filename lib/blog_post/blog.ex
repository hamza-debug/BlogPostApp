defmodule BlogPost.Blog do
  ...
  """
 def get_current_user_post(id) do
     query =  from p in Post,
               join: u in User,
               on: u.id == p.user_id,
               where: p.user_id == ^id,
               select: %Post{ id: p.id,body: p.body, name: p.name , user_id: p.user_id, user: %User{id: u.id, first_name: u.first_name}}
     Repo.all(query)
  end
